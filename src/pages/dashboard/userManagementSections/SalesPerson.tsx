...
const SalesPerson: React.FC = () => {
  ...
  const fetchSalePersonUsers = async () => {
    try {
      setLoading(true);
      const API_URL =
      ...
      toast.error("An error occurred while fetching Sales users");
    } finally {
      setLoading(false);
    }
  };
  ...
  useEffect(() => {
    fetchSalePersonUsers();
  }, []);
  const navigate = useNavigate();
  // Transform the real data instead of using static adminData
  const formattedSalesUsers = users.map((user, index) => ({
    id: user._id,
    userId: user.userId,
  ...
  return (
    <div className="mx-auto px-3 py-6 sm:px-4 md:px-6 xl:px-8 xl:py-6 w-full">
      ...
      {loading ? (
        ...
        <GenericTable
          columns={columns}
          data={formattedSalesUsers}
          tableTitle="Sale Person"
