...
//   const existingLogoUrl = ""; // Set this to your existing logo URL if available
...
//   //     formDataToSend.append("companyCurrency", currency);

//   //     // Add social media links as array
//   //     const socialMediaLinks = [];
//   //     if (facebook.trim()) socialMediaLinks.push(facebook.trim());
//   //     if (instagram.trim()) socialMediaLinks.push(instagram.trim());
//   //     if (twitter.trim()) socialMediaLinks.push(twitter.trim());
//   //     if (linkedin.trim()) socialMediaLinks.push(linkedin.trim());
//   //     if (whatsapp.trim()) socialMediaLinks.push(whatsapp.trim());

//   //     // Append each social media link separately (as the backend expects array format)
//   //     socialMediaLinks.forEach((link) => {
//   //       formDataToSend.append("socialMediaLinks", link);
//   //     });
...
//   //       `${API_URL}/api/abid-jewelry-ms/createSystemSetting`,
//   //       formDataToSend,
//   //       {
//   //         headers: {
...
//   //     } else {
//   //       toast.error(response.data.message || "Failed to save system settings");
//   //     }
//   //   } catch (error: any) {
...
//         // Reset all form fields and image after successful submission
//         setCompanyName("");
//         setPhoneNumber("");
//         setAddress("");
//         setEmail("");
...
//                 className="w-full border border-gray-300 rounded-md p-2 appearance-none outline-none focus:outline-none mt-1"
//               >
...
export default function SystemSettingRenderPage() {
  ...
  //     if (response.data.success) {
  //       const isUpdate = systemSettingData?.data?.length > 0;
  //       toast.success(
  //         isUpdate
  ...
  //       // Only reset form if it was a create operation
  //       if (!isUpdate) {
  //         setCompanyName("");
  //         setPhoneNumber("");
  //         setAddress("");
  ...
  const handleSubmit = async () => {
    try {
      ...
      const isUpdate = systemSettingData?.data?.length > 0;
      const systemSettingId = isUpdate ? systemSettingData.data[0]._id : null;

      // Choose API endpoint and method based on operation type
      const apiUrl = isUpdate
        ? `${API_URL}/api/abid-jewelry-ms/updateSystemSetting/${systemSettingId}`
        : `${API_URL}/api/abid-jewelry-ms/createSystemSetting`;

      const apiMethod = isUpdate ? "put" : "post";

      // Make API request
      const response = await axios[apiMethod](apiUrl, formDataToSend, {
        headers: {
          "x-access-token": token,
          "Content-Type": "multipart/form-data",
        },
      ...
      if (response.data.success) {
        ...
        if (!isUpdate) {
          ...
          // For update, refresh the data to get the latest information
          ...
          if (
            ...
            setSystemSettingData(updatedResponse.data);
            const updatedData = updatedResponse.data.data[0];

            // Update existing logo URL if changed
            if (updatedData.companyLogo) {
              setExistingLogoUrl(`${API_URL}${updatedData.companyLogo}`);
            }
  ...
  // Handle form reset/cancel
  const handleCancel = () => {
    if (systemSettingData?.data?.length > 0) {
      // If data exists, restore original values
      const data = systemSettingData.data[0];
      setCompanyName(data.companyName || "");
      setPhoneNumber(data.phoneNumber || "");
      setAddress(data.address || "");
      setEmail(data.email || "");
      ...
      // If no data exists, clear all fields
      setCompanyName("");
      setPhoneNumber("");
      setAddress("");
      setEmail("");
