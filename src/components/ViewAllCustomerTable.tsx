...
//   // Update the fetchCustomerDetails function:
...
//         `${API_URL}/api/abid-jewelry-ms/getOneCustomer/${customerId}`,
//         {
//           headers: {
...
//     try {
//       const customerDetails = await fetchCustomerDetails(row.id);
...
//           phone: customerDetails.phone || "", // Fix: use 'phone' instead of 'phoneNo'
//           dob: customerDetails.dateOfBirth
//             ? new Date(customerDetails.dateOfBirth).toISOString().split("T")[0]
//             : "",
//           address: customerDetails.address || "",
//           segmentation: customerDetails.segmentation,
//         });
//         setIsEditing(true);
//       }
//     } catch (error) {
//       console.error("Error in edit flow:", error);
//       toast.error("Failed to prepare customer data for editing");
//     }
//   };

//   // Update the handleUpdateCustomer function validation:
//   const handleUpdateCustomer = async () => {
//     if (!editingRow || !formData) return;
...
//     }
//     if (!formData.phone) {
//       // Fix: use 'phone' instead of 'phoneNo'
...
//         phone: formData.phone, // Fix: use 'phone'
//         dateOfBirth: formData.dob, // Fix: use 'dateOfBirth'
//         address: formData.address,
//         segmentation: formData.segmentation,
//       };

//       const response = await axios.put(
//         `${API_URL}/api/abid-jewelry-ms/updateCustomer/${editingRow.id}`,
//         updateData,
//         {
//           headers: {
//             "x-access-token": token,
...
//         setIsEditing(false);
//         // Refresh the customer list or update the local state
...
//   // Make sure the handleFormChange function handles all field names correctly:
...
//   // Handle segmentation change in edit modal
...
//         setDeleteModal(true);
//         // After showing the success modal, you might want to refresh the data
//         // This could be done by emitting an event or using a callback
//       } else {
...
//               <Dropdown
//                 options={rowsPerPageOptions.map((num) => `${num} Row`)}
//                 onSelect={(val) => {
//                   const selected = parseInt(val.split(" ")[0]);
//                   setRowsPerPage(selected);
//                   setCurrentPage(1);
//                 }}
//                 defaultValue={`${rowsPerPage} Row`}
...
//                 onClick={() => {
//                   setDeleteModal(false);
//                   window.location.reload(); // Refresh the page to update the customer list
//                 }}
...
const ViewAllCustomerTable: React.FC<ViewAllCustomerTableProps> = ({
  ...
  // Update the handleUpdateCustomer function:
  const handleUpdateCustomer = async () => {
    if (!editingRow || !formData) return;
