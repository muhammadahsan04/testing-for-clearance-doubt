...
const ProductCategoryTable: React.FC<ProductCategoryTableProps> = ({
  ...
  const [loadingCategoryDetails, setLoadingCategoryDetails] =
  ...
  // Fetch category details when edit button is clicked
  const fetchCategoryDetails = async (categoryId: string) => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        // Update form data with category details
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await axios.put(
        `${API_URL}/api/abid-jewelry-ms/updateCategory/${editingRow._id}`,
        payload,
        {
          headers: {
            "x-access-token": token,
  ...
  const handleEditClick = (e: React.MouseEvent, row: any) => {
    e.stopPropagation();
    if (!canUpdate) {
      toast.error("You don't have permission to edit product category");
      return;
    }
    setIsEditing(true);
    setEditingRow(row);
    fetchCategoryDetails(row._id);
  };
  ...
  const handleDeleteClick = (e: React.MouseEvent, row: any) => {
    e.stopPropagation();
    if (!canDelete) {
      toast.error("You don't have permission to delete product category");
      return;
    }
  ...
  const handleRowClick = (row: any) => {
    setSelectedUser(row);
    if (!canUpdate) {
      return; // Don't open modal if no permission
    }
    if (onRowClick) {
      onRowClick(row);
    }
              ...
              <tr className="font-semibold text-[16px] whitespace-nowrap w-full">
                {columns.map((col, index) => {
                  ...
                  return (
                    <th
                      key={col.accessor}
                      className="px-12 py-3 whitespace-nowrap"
                      style={{
