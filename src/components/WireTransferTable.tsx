...
export function WireTransferTable({ transactions }: { transactions: Transaction[] }) {
  ...
  const filteredData = transactions.filter((transaction) => {
    ...
    const matchesPaymentMode = paymentMode === "All" || transaction.paymentMode === paymentMode
    ...
    if (dateRange && dateRange[0] && dateRange[1]) {
      ...
      matchesDateRange =
        ...
        (transactionDate.isSame(dateRange[0], "day") &&
          (transactionDate.isBefore(dateRange[1], "day") || transactionDate.isSame(dateRange[1], "day")))
    }
  ...
  return (
    <div className="bg-white rounded-lg shadow-sm p-4 sm:p-6">
      <div className="flex flex-wrap justify-between items-center mb-6">
        <h2 className="text-2xl font-bold text-[#056BB7]">Wire Transfer</h2>
        <button className="bg-gray-600 hover:bg-gray-700 text-white px-4 py-1 rounded transition-colors">Export</button>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-6 items-end">
        <div>
          ...
          <RangePicker className="h-10 !text-gray-400 !border-gray-300 w-full" onChange={handleDateRangeChange} />
        </div>

        {/* <div className="flex items-end border"> */}
          {/* <label className="block text-sm font-medium text-gray-700 mb-1">Payment Mode</label> */}
          <Dropdown
            options={["All", "Cash", "Bank Transfer", "Wire Transfer", "Apple Pay"]}
        ...
        <table className="w-full">
          <thead>
            <tr className="bg-gray-100 font-semibold text-[16px] whitespace-nowrap w-full">
              ...
              <th className="px-4 py-3 text-sm font-medium text-gray-700 text-center">Type</th>
              {/* <th className="px-4 py-3 text-left text-sm font-medium text-gray-700">Payment Mode</th> */}
              <th className="px-4 py-3 text-sm font-medium text-gray-700 text-center">Money In ($)</th>
              <th className="px-4 py-3 text-sm font-medium text-gray-700 text-center">Money Out ($)</th>
              <th className="px-4 py-3 text-sm font-medium text-gray-700 text-center">Balance ($)</th>
            </tr>
          </thead>
          <tbody className="bg-white divide-y divide-gray-200">
            {currentData.map((transaction) => (
              <tr key={transaction.id} className="hover:bg-gray-50 whitespace-nowrap cursor-pointer">
                <td className="px-4 py-3 text-sm text-gray-900 text-center">{transaction.date}</td>
                <td className="px-4 py-3 text-sm text-gray-900">{transaction.description}</td>
                <td className="px-4 py-3 text-sm text-gray-900 text-center">{transaction.type}</td>
                {/* <td className="px-4 py-3 text-sm text-gray-900">{transaction.paymentMode}</td> */}
                <td className="px-4 py-3 text-sm text-gray-900 text-center">
                  {transaction.moneyIn ? transaction.moneyIn.toLocaleString() : "--"}
                </td>
                <td className="px-4 py-3 text-sm text-gray-900 text-center">
                  {transaction.moneyOut ? transaction.moneyOut.toLocaleString() : "--"}
                </td>
